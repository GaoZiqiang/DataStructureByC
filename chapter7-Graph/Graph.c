...
typedef int InfoType; // 网的权值   
typedef char VertexType[MAX_NAME]; // 顶点字符串类型   

typedef enum {
    DG, DN, AG, AN
} GraphKind; // 枚举{有向图,有向网,无向图,无向网}   
...
} VNode, AdjList[MAX_VERTEX_NUM]; // 头结点（保存顶点值的数组）   
...
int CreateGraph(ALGraph *G) {
    ...
    {
        scanf("%s", (*G).vertices[i].data); // 将顶点值放到数组中
        (*G).vertices[i].firstarc = NULL;
    }
    if ((*G).kind == 1 || (*G).kind == 3) // 若为网，添加权值  
...
VertexType* GetVex(ALGraph G, int v) {
    if (v >= G.vexnum || v < 0)
        exit(0);
    return &G.vertices[v].data; // 返回v的值
}
